...
int main() {
	...
	while (1) {
		...
		if (pfd.revents & POLLIN) {
			stream = nm_nextpkt(nmr, &h);//用于接收网卡接收数据包
			eh = (struct ethhdr*)stream;//解析包头
			...
			if (ntohs(eh->h_proto) == PROTO_IP) {
				...
				*/
				...
				//如果是ARP数据包给本机发的我就封装一个arp数据包写入mac地址返回给他
				if (arp->arp.dip == inet_addr("192.168.189.137")) {
					echo_arp_pkt(arp, &arp_rt, "00:50:56:33:1c:ca");
					/*
					    nm_inject()是用来往共享内存中写入待发送的数据包数据的。
					    数据包经共享内存拷贝到网卡，然后发送出去。所以nm_inject()是用来发包的。
					*/
